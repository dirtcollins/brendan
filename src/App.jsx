      ...
      <div className={`input-with-unit ${unit ? "fraction-input" : ""}`}>
        <input
          ...
          onKeyDown={(event) => {
            ...
            if (unit && event.key === "ArrowUp") {
              event.preventDefault();
              stepValue(1);
            }
            if (unit && event.key === "ArrowDown") {
              event.preventDefault();
              stepValue(-1);
            }
          }}
...
function FenceConstructionPlans({ settings, calc, segments, pad, scale, svgW, fenceTop, fenceHeight, postW, picketW, railH, onPointerDown, dragging }) {
  ...
  return (
    <g className="construction-plan">
      ...
      {segments.filter((segment) => segment.type === "section").map((segment) => (
      ...
      {segments
        .filter((segment) => segment.type === "gate")
        .map((segment) => (
          <g key={`plan-gate-${segment.index}`} className={`gate-drag-target ${dragging ? "dragging" : ""}`} onPointerDown={onPointerDown}>
...
function PlanFenceSection({ segment, settings, scale, y, height, picketW, railH, railYs }) {
  return (
    <g>
      ...
      <HorizontalDimension x1={segment.x} x2={segment.x + segment.width} y={y + height + 28} label={`SECTION ${segment.index} ${feet(segment.length)}`} />
    </g>
  );
}
  ...
  return (
    <g>
      ...
      {segments
        .filter((segment) => segment.type === "gate")
        .map((segment) => (
          <g key={`top-gate-${segment.index}`} className={`gate-drag-target ${dragging ? "dragging" : ""}`} onPointerDown={onPointerDown}>
  ...
  return (
    <g transform="translate(20 -6) skewX(-6)">
      ...
      {segments
        .filter((segment) => segment.type === "gate")
        .map((segment) => (
          <g key={`perspective-gate-${segment.index}`} className={`gate-drag-target ${dragging ? "dragging" : ""}`} onPointerDown={onPointerDown}>
